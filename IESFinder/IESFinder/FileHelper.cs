...
namespace IESFinder
{
    ...
    class FileHelper
    {
        private static string pathOriginalFile = AppDomain.CurrentDomain.BaseDirectory + "..\\..\\..\\data\\CADASTRO_DAS_IES_2011.csv";            

        private static string pathBtree = AppDomain.CurrentDomain.BaseDirectory + "..\\..\\..\\data\\arvore.bin";
        ...
        {  
            ...
            using (StreamReader ies_stream = new StreamReader(pathOriginalFile, Encoding.GetEncoding(CultureInfo.GetCultureInfo("pt-BR").TextInfo.ANSICodePage)))
            {
                try
                {
                ...
                using (var tree = new BPlusTree<int, DadosIES>(tree_options))
                {
                    ...
                    { 
                        switch(criterio)
                        {
                            ...
                            case nameof(IES.SIGLA):
                                if (par.Value.ies.SIGLA.IndexOf(valor, StringComparison.OrdinalIgnoreCase) >= 0)
                                {
                                    lista.Add(par.Value);
                                }
                                break;
        ...
        public static void RemoveBinFile()
        {
            if (File.Exists(pathBtree))
            {
                File.Delete(pathBtree);
            }
        }
    }
}
